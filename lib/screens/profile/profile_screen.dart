...
import 'package:suvic_app/screens/profile/components/address_user.dart';
import 'package:suvic_app/screens/profile/components/header.dart';
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void initState() {
    ...
    rgController.text =
        userController.user?.rg == null ? '' : '${userController.user?.rg}';
    cpfController.text = userController.user?.cpf ?? '';
    birthdateController.text = userController.user!.birthdate == null
        ? ''
        : DateFormat("dd/MM/yyyy")
            .format(DateTime.parse(userController.user!.birthdate!));
    sexController = userController.user?.sex == "NS"
        ? "Não especificado"
        : userController.user?.sex ?? 'Não especificado';
    healthProfessional =
        userController.user!.healthProfessional! ? 'Sim' : 'Não';
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    InkWell(
                      onTap: () {
                        perfilController.statePerfil.value =
                            StatePerfil.ADDRESS;
                      },
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.end,
              ...
              onPressed: () async {
                ...
                if (formKey.currentState!.validate()) {
                  ...
                  if (rgController.text.isNotEmpty) {
                    int rg = int.parse(rgController.text
                        .replaceAll(".", "")
                        .replaceAll("-", ""));
                  ...
                  userController.user?.birthdate =
                      "${listData.last}-${listData[1]}-${listData.first}";
                  userController.user?.sex = sexController == "Não especificado"
                      ? "NS"
                      : sexController;
                  userController.user?.healthProfessional =
                      healthProfessional == "Sim" ? true : false;
