                 ...
                 child: Column(
                   children: [
                     ...
                     Obx(() {
                       return Row(
                         children: [
                           creditCardController.pagarMeModel.value.cardNumber == null
                               ? FlatButton(
                             ...
                             child: Text(
                               ...
                               : Text(
                             "Número Cartão: ********${creditCardController.pagarMeModel.value.cardNumber?.substring(16 - 3)}",
                     ...
                     ButtonCustom(
                         onPressed: () {
                           if( creditCardController.pagarMeModel.value.cardNumber == null){
                             keyScaffold.currentState?.showSnackBar(SnackBar(content: Text("Adicione um Cartãoo de Credito"),));
                           }else{
                             if(_keyForm.currentState!.validate()){
                               creditCardController.pagarMeModel.value.items![0]!.quantity = 1;
                               creditCardController.pagarMeModel.value.items![0]!.id = "1";
                               creditCardController.pagarMeModel.value.amount = int.parse(valueController.text) * 100;
                               creditCardController.pagarMeModel.value.items![0]!.unitPrice = int.parse(valueController.text) * 100;
                               creditCardController.pagarMeModel.value.items![0]!.title = productController.text;
          ...
          case StateProductCard.SUCCESS:
           return Center(
             child: Column(
               ...
               children: [
                 Text("Transação realizada com sucesso", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
          ...
          case StateProductCard.FAIL:
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text("Transação recusada, tente novamente mais tarde", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),),
